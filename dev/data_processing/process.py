...
def simplify_nq_example(nq_example):
  ...
  `tokens` fields. All annotations in the ouput are represented as
  ...
  Returns:
    ...
    `tokens` or `document_html`, and with all annotations represented
...
#   print(nq_example["document"]["tokens"])
#   print(type(nq_example["document"]["tokens"]))
  text = " ".join([_clean_token(nq_example["document"]["tokens"])])
  print(text)
  ...
  simplified_nq_example = {
      "question_text": nq_example["question"]["text"],
      "example_id": nq_example["id"],
      "document_url": nq_example["document"]["url"],
      "document_text": text,
      "long_answer_candidates": [
  ...
  if len(get_nq_tokens(simplified_nq_example)) != len(
      nq_example["document"]["tokens"]["token"]):
